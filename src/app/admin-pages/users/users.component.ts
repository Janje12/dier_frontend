...
export class UsersComponent implements OnInit {
  ...
  constructor(private korisnikService: UserService, private toastrService: NbToastrService,
              private router: Router) {
  }

  ngOnInit(): void {
    this.korisnikService.getUsers('').subscribe(k => {
  ...
  createUser({newData: user, confirm: confirm}): void {
    try {
      this.korisnikService.createUser(user).subscribe(u => {
      });
      confirm.resolve();
  ...
  updateUser({newData: user, confirm: confirm}): void {
    try {
      this.korisnikService.updateUser(user, user._id).subscribe(u => {
      });
      confirm.resolve();
  ...
  deleteUser({data: user, confirm: confirm}): void {
    try {
      this.korisnikService.deleteUser(user, user._id).subscribe(u => {
      });
      confirm.resolve();
